...
void tampilData(Direktur direktur[])
{
	...
	for(i=0; i<maxDirektur; i++)
		if(strcmpi(direktur[i].nama,"-")!=0) //kalau tidak kosong akan di outputkan
		{
			...
			{
				printf("\n\n\t\t\t===++ Data Trainee ++===");
				printf("\n\t\tNama Trainee\t\t: %s", direktur[i].karyawan.nama);
				printf("\n\t\tJam Latihan Trainee\t: %d", direktur[i].karyawan.jamKerja);
				printf("\n\t\tGaji Trainee\t\t: Rp%.2f", direktur[i].karyawan.totalGaji);
			}
			else
...
Direktur updateDirektur(String nama, String NIP, int levelMinimarket, Karyawan karyawan)
{
	...
	if(levelMinimarket>0 && levelMinimarket<4)
		temp.karyawan.totalGaji = temp.karyawan.jamKerja * 50000;
	else if(levelMinimarket>3 && levelMinimarket<7)
		temp.karyawan.totalGaji = temp.karyawan.jamKerja * 70000;
	else if(levelMinimarket>6)
		temp.karyawan.totalGaji = temp.karyawan.jamKerja * 90000;
